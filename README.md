...
Bash
    npm install
    ```
2.  O arquivo `.env` já está configurado na porta `3000`. Inicie o servidor:
```bash
    npm start
    ```
...
```bash
    ...
    ```
2.  Instale as dependências de interface e 3D:
```bash
    npm install
    ```
3.  Execute o servidor do Vite expondo-o para a rede local:
```bash
    npm run dev -- --host
    ```
